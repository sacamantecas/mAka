...
afm = 3.6/2 ; // agujero de fijaci+好 del motor
asm = 1 ; // arandela de separaci+好 del motor
rasm = 8/2 ; // radio de la arandela de separacion del motor
afme = 6.4/2 ; // apertura para la cabeza del tornillo de fijaci+好 del motor
pafme = 1 ; // del agujero del tornillo penetraci+好
...
module pata() {
	...
	difference() {
		...
		difference() { // le quito un trozo para no da鎙r el soporte del agujero de la varilla X
			union() {
					...
					for (lado_y=[-1,1])
						translate([lado_x*rodamientos_dx/2 - zocalo_dx/2, lado_y*rodamientos_dy/2 - zocalo_dy/2, 0])
							difference() {
								...
								// hacer un recorte al z鏂alo, porque pega con la pata
